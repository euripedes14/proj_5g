...
labels_ul = torch.tensor(data_ul["labels"], dtype=torch.float32).unsqueeze(1)

sequences_dl = torch.tensor(data_dl["sequences"], dtype=torch.float32)
labels_dl = torch.tensor(data_dl["labels"], dtype=torch.float32).unsqueeze(1)
...
input_dim = max(sequences_ul.shape[-1], sequences_dl.shape[-1])
